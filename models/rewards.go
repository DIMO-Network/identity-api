...
func (rewardL) LoadAftermarketToken(ctx context.Context, e boil.ContextExecutor, singular bool, maybeReward interface{}, mods queries.Applicator) error {
	...
	if singular {
		...
		if !queries.IsNil(object.AftermarketTokenID) {
			args = append(args, object.AftermarketTokenID)
		}
	...
	Outer:
		for _, obj := range slice {
			...
			for _, a := range args {
				if queries.Equal(a, obj.AftermarketTokenID) {
					continue Outer
				}
			}

			if !queries.IsNil(obj.AftermarketTokenID) {
				args = append(args, obj.AftermarketTokenID)
			}
	...
	query := NewQuery(
		...
		qm.WhereIn(`identity_api.aftermarket_devices.id in ?`, args...),
	)
	if mods != nil {
		mods.Apply(query)
	}
...
func (rewardL) LoadSyntheticToken(ctx context.Context, e boil.ContextExecutor, singular bool, maybeReward interface{}, mods queries.Applicator) error {
	...
	if singular {
		...
		if !queries.IsNil(object.SyntheticTokenID) {
			args = append(args, object.SyntheticTokenID)
		}
	...
	Outer:
		for _, obj := range slice {
			...
			for _, a := range args {
				if queries.Equal(a, obj.SyntheticTokenID) {
					continue Outer
				}
			}

			if !queries.IsNil(obj.SyntheticTokenID) {
				args = append(args, obj.SyntheticTokenID)
			}
	...
	query := NewQuery(
		...
		qm.WhereIn(`identity_api.synthetic_devices.id in ?`, args...),
	)
	if mods != nil {
		mods.Apply(query)
	}
...
func (o *Reward) Upsert(ctx context.Context, exec boil.ContextExecutor, updateOnConflict bool, conflictColumns []string, updateColumns, insertColumns boil.Columns) error {
	...
	if !cached {
		...
		cache.query = buildUpsertQueryPostgres(dialect, "\"identity_api\".\"rewards\"", updateOnConflict, ret, update, conflict, insert)
