	...
	if !cached {
		...
		if len(conflict) == 0 && updateOnConflict && len(update) != 0 {
			if len(dcnPrimaryKeyColumns) == 0 {
				return errors.New("models: unable to upsert dcns, could not build conflict column list")
			}

			conflict = make([]string, len(dcnPrimaryKeyColumns))
			copy(conflict, dcnPrimaryKeyColumns)
		}
		cache.query = buildUpsertQueryPostgres(dialect, "\"identity_api\".\"dcns\"", updateOnConflict, ret, update, conflict, insert, opts...)
...
func (o DCNSlice) DeleteAll(ctx context.Context, exec boil.ContextExecutor) (int64, error) {
	...
	sql := "DELETE FROM \"identity_api\".\"dcns\" WHERE " +
		strmangle.WhereInClause(string(dialect.LQ), string(dialect.RQ), 1, dcnPrimaryKeyColumns, len(o))
