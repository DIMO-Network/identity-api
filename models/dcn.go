...
type DCN struct {
	...
	ResolverAddress null.Bytes `boil:"resolver_address" json:"resolver_address,omitempty" toml:"resolver_address" yaml:"resolver_address,omitempty"`
...
var DCNTableColumns = struct {
	...
	ResolverAddress: "dcn.resolver_address",
...
var DCNWhere = struct {
	...
	ResolverAddress whereHelpernull_Bytes
	Expiration      whereHelpernull_Time
}{
	Node:            whereHelper__byte{field: "\"identity_api\".\"dcn\".\"node\""},
	OwnerAddress:    whereHelper__byte{field: "\"identity_api\".\"dcn\".\"owner_address\""},
	ResolverAddress: whereHelpernull_Bytes{field: "\"identity_api\".\"dcn\".\"resolver_address\""},
	Expiration:      whereHelpernull_Time{field: "\"identity_api\".\"dcn\".\"expiration\""},
}
...
var (
	dcnAllColumns            = []string{"node", "owner_address", "resolver_address", "expiration"}
	dcnColumnsWithoutDefault = []string{"node", "owner_address"}
	dcnColumnsWithDefault    = []string{"resolver_address", "expiration"}
...
func DCNS(mods ...qm.QueryMod) dcnQuery {
	mods = append(mods, qm.From("\"identity_api\".\"dcn\""))
	q := NewQuery(mods...)
	if len(queries.GetSelect(q)) == 0 {
		queries.SetSelect(q, []string{"\"identity_api\".\"dcn\".*"})
	}
...
func FindDCN(ctx context.Context, exec boil.ContextExecutor, node []byte, selectCols ...string) (*DCN, error) {
	...
	query := fmt.Sprintf(
		"select %s from \"identity_api\".\"dcn\" where \"node\"=$1", sel,
	)
...
func (o *DCN) Insert(ctx context.Context, exec boil.ContextExecutor, columns boil.Columns) error {
	...
	if !cached {
		...
		if len(wl) != 0 {
			cache.query = fmt.Sprintf("INSERT INTO \"identity_api\".\"dcn\" (\"%s\") %%sVALUES (%s)%%s", strings.Join(wl, "\",\""), strmangle.Placeholders(dialect.UseIndexPlaceholders, len(wl), 1, 1))
		} else {
			cache.query = "INSERT INTO \"identity_api\".\"dcn\" %sDEFAULT VALUES%s"
		}
...
func (o *DCN) Update(ctx context.Context, exec boil.ContextExecutor, columns boil.Columns) (int64, error) {
	...
	if !cached {
		...
		cache.query = fmt.Sprintf("UPDATE \"identity_api\".\"dcn\" SET %s WHERE %s",
...
func (o DCNSlice) UpdateAll(ctx context.Context, exec boil.ContextExecutor, cols M) (int64, error) {
	...
	sql := fmt.Sprintf("UPDATE \"identity_api\".\"dcn\" SET %s WHERE %s",
...
func (o *DCN) Upsert(ctx context.Context, exec boil.ContextExecutor, updateOnConflict bool, conflictColumns []string, updateColumns, insertColumns boil.Columns) error {
	...
	if !cached {
		...
		cache.query = buildUpsertQueryPostgres(dialect, "\"identity_api\".\"dcn\"", updateOnConflict, ret, update, conflict, insert)
...
func (o *DCN) Delete(ctx context.Context, exec boil.ContextExecutor) (int64, error) {
	...
	sql := "DELETE FROM \"identity_api\".\"dcn\" WHERE \"node\"=$1"
...
func (o DCNSlice) DeleteAll(ctx context.Context, exec boil.ContextExecutor) (int64, error) {
	...
	sql := "DELETE FROM \"identity_api\".\"dcn\" WHERE " +
...
func (o *DCNSlice) ReloadAll(ctx context.Context, exec boil.ContextExecutor) error {
	...
	sql := "SELECT \"identity_api\".\"dcn\".* FROM \"identity_api\".\"dcn\" WHERE " +
		strmangle.WhereClauseRepeated(string(dialect.LQ), string(dialect.RQ), 1, dcnPrimaryKeyColumns, len(*o))
...
func DCNExists(ctx context.Context, exec boil.ContextExecutor, node []byte) (bool, error) {
	var exists bool
	sql := "select exists(select 1 from \"identity_api\".\"dcn\" where \"node\"=$1 limit 1)"
