...
func TestHandleAftermarketDeviceNodeMintedEvent(t *testing.T) {
	...
	assert.Equal(t, aftermarketDeviceNodeMintedArgs.Owner.Bytes(), ad.Owner)
	assert.Equal(t, aftermarketDeviceNodeMintedArgs.Owner.Bytes(), ad.Beneficiary.Bytes)
	assert.Equal(t, mintedAt.UTC().Format(time.RFC3339), ad.MintedAt.UTC().Format(time.RFC3339))
}
...
func TestHandleAftermarketDeviceAttributeSetEvent(t *testing.T) {
	...
	prepopulateAttribute := models.AftermarketDevice{
		...
		MintedAt: mintedAt,
		Owner:    common.Hex2Bytes("46a3A41bd932244Dd08186e4c19F1a7E48cbcDf4"),
	}
	...
	assert.Equal(t, prepopulateAttribute.Owner, ad.Owner)
	assert.Equal(t, null.Bytes{Bytes: []uint8{}}, ad.Beneficiary)
	assert.Equal(t, prepopulateAttribute.MintedAt.UTC().Format(time.RFC3339), ad.MintedAt.UTC().Format(time.RFC3339))
}
...
func TestHandleAftermarketDevicePairedEvent(t *testing.T) {
	...
	assert.Equal(t, aftermarketDevicePairData.Owner.Bytes(), ad.Owner)
...
func TestHandleAftermarketDeviceTransferredEvent(t *testing.T) {
	...
	d := models.AftermarketDevice{
		ID:          100,
		Owner:       aftermarketDeviceTransferredData.OldOwner.Bytes(),
	...
	assert.Equal(t, aftermarketDeviceTransferredData.NewOwner, common.BytesToAddress(ad.Owner))
