...
func (s *VehiclesPrivilegesRepoTestSuite) Test_GetVehiclePrivileges_Success() {
	...
	res, err := s.repo.GetPrivilegesForVehicle(s.ctx, 1, model.PrivilegeFilterBy{})
	s.NoError(err)
...
func (s *VehiclesPrivilegesRepoTestSuite) Test_Privileges_NoExpiredPrivilege_Pagination() {
	...
	limit := 2
	res, err := s.repo.GetPrivilegesForVehicle(s.ctx, 1, model.PrivilegeFilterBy{First: &limit})
	s.NoError(err)
