...
func GetSyntheticDeviceByID(ctx context.Context, syntheticDevice int) (*gmodel.SyntheticDevice, error) {
	// read loader from context
	loaders := ctx.Value(dataLoadersKey).(*Loaders)
	// invoke and get thunk
	thunk := loaders.SyntheticDeviceByID.Load(ctx, syntheticDevice)
	// read value from thunk
	return thunk()
}
